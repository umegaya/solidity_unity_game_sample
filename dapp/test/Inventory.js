...
contract('Inventory', (accounts) => {
    it("can create and load cat", () => {
        ...
        //store / load cat data
            ...
            return c.addFixedCat(accounts[0], NAME, HP, ATK, DEF, SKILL_IDS, true);
        }).then((ret) => {
        ...
        //breed cat
            ...
            return c.addFixedCat(accounts[0], NAME, HP, ATK2, DEF2, SKILL_IDS2, false);
        }).then((ret) => {
            var log = ret.logs[0];
            assert.equal(log.event, 'AddCat', "event should be AddCat");
            assert.equal(log.args.id, 2, "cat id should be correct");
            return c.estimateBreedValue(accounts[0], 2, accounts[1], 1, 0);
        }).then((ret) => {
